...
const Terms = () => {
  ...
  const onSingleCheck = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCheckBoxData((prevData) =>
      prevData.map((item) => (targetId === item.value ? { ...item, isChecked: !item.isChecked } : item)),
    );

    const isAllChecked = checkBoxData.every((item) => item.isChecked);
    setAllIsChecked(isAllChecked);
  };
  ...
  const onAllCheck = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setCheckBoxData((prevData) => prevData.map((item) => ({ ...item, isChecked })));
  };
      ...
      <div className="m-auto flex w-[360px] flex-col items-center gap-4 px-2 py-[18px] pb-[130px] pt-10 md:w-[752px] md:px-[26px] md:py-[60px]">
        ...
        <div className="flex w-[319px] flex-col items-center gap-[30px] p-2.5 md:w-[680px] md:p-2.5">
          <div className="flex flex-shrink-0 flex-col items-start justify-center gap-1.5 self-stretch">
            <CheckBox title={allCheckBoxData[0].title} isChecked={allIsChecked} onCheck={onAllCheck} />
          ...
          <div className="flex flex-col gap-[30px]">
            {checkBoxData.map((item) => (
              <>
                <div className="flex flex-shrink-0 flex-col items-start justify-center gap-1.5 self-stretch">
                  <CheckBox key={item.id} title={item.value} isChecked={item.isChecked} onCheck={onSingleCheck} />
        ...
        <PrimaryButton label="다음 단계" disabled={!allIsChecked} onClick={() => router.push('/auth/sign-up')} />
      </div>
    </>
  );
};
