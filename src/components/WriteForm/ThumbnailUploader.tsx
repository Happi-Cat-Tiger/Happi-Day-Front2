...
const ThumbnailUploader = () => {
  ...
  const handleChangeThumbnail = (value: File, writingInfoValue: any) => {
    setWritingInfoValue({
      ...writingInfoValue,
  ...
  const handleDrop = useCallback(
    (e: DragEvent<HTMLLabelElement>, setActive?: Dispatch<SetStateAction<boolean>>, writingInfoValue?: any) => {
      ...
      if (file && file.type.includes('image')) {
        handleChangeThumbnail(file, writingInfoValue);
      }
    ...
    <div className="flex flex-col gap-2">
      ...
      <form className="flex w-full flex-col items-center justify-center gap-5">
        ...
        <div className="mx-auto w-2/3 rounded-lg border-2 border-gray-300 bg-gray-100 p-5">
          {thumbnailImage ? (
            <img src={thumbnailImage ? URL.createObjectURL(thumbnailImage) : ''} className="mx-auto h-auto w-[250px]" />
          ) : (
